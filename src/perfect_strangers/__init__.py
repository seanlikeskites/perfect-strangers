...
def create_matcher(groups_per_round: int, group_size: int, participant_labels: ParticipantLabels=None) -> BaseMatcher:
    ...
    """
    lookup_matcher = LookupMatcher.create_matcher(groups_per_round, group_size, participant_labels)
    ...
    if algo_matcher is None and use_finite_plane_construction(groups_per_round, group_size):
        algo_matcher = FinitePlaneMatcher.create_matcher(groups_per_round, group_size, participant_labels)

    # Default to column shift matching.
    if algo_matcher is None:
        algo_matcher = ColumnShiftMatcher(groups_per_round, group_size, participant_labels)
